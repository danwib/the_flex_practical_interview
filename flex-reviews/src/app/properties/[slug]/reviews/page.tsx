...
type Review = {
  id: number; publicReview: string; listingName: string; guestName: string;
  reviewCategory: { category: string; rating: number | null }[];
};

export default function PublicReviews({ params }: { params: { slug: string } }) {
  ...
  const approved = useMemo(() => reviews.filter(r => approvals[r.id]), [reviews, approvals]);
      ...
      <ul className="space-y-4">
        {approved.map(r => (
          <li key={r.id} className="border rounded p-4">
            <p className="text-sm text-neutral-600">{r.guestName}</p>
            <p className="mt-2">{r.publicReview}</p>
            <div className="text-xs text-neutral-500 mt-2">
              {r.reviewCategory?.map(c => <span key={c.category} className="mr-3">{c.category}: <b>{c.rating ?? '-'}</b></span>)}
            </div>
          </li>
        ))}
      </ul>
